...
Current preparation: **1.0.0+7**, the first stable version; publication pending.
The maintainer has accepted RC.1. See [stable 1.0.0](v1.0.0.md) for artifact
paths, prior evidence and the exact stable-artifact verification still required.
...
3. From the clean preparation commit, run `tool/release_android.sh` with JDK 21.
   ...
   [stable checklist](v1.0.0.md#stable-artifact-handoff-checklist).
...
Samsung/Fire verification of each newly versioned artifact explicitly pending
until those devices are retested. The maintainer confirmed pre-RC startup and
primary flows and subsequently reported completed RC testing; that acceptance
does not certify the newly versioned stable artifact.

The workflow writes `build/app/outputs/flutter-apk/app-production-release.apk`,
then `artifacts/zanka-no-tachi-v1.0.0.apk` and its `.sha256` for stable 1.0.0.
It does not commit, push, tag, upload or create a GitHub Release. Check Android
`versionName=1.0.0` and `versionCode=7`, not just the filename. Retain the
APK checksum and corresponding R8 mapping privately with the artifact.
...
transitive dependencies remain packaged. Stable 1.0.0 changes neither database
schema 6 nor backup format 3. Older supported backups remain readable; exact engine
and Video Display Mode preferences stay device-local during restore.

Beta.2 and later production-signed builds, including RC.1, update in place with
the permanent fingerprint documented above. Never uninstall or clear an existing production
installation for release testing. The old debug-signed beta.1 has a separate
[migration guide](BETA1_TO_BETA2_MIGRATION.md), not a stable update procedure.

Publication is a separate, explicitly authorized action after stable-artifact
verification. This preparation creates no tag or GitHub Release and uploads no
APK. Wait for the maintainer's explicit approval after installing and verifying
the exact stable build. If later authorized,
