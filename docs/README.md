# Documentation

Public technical and product documentation is grouped by purpose.

## Architecture

- [Canonical domain model](architecture/DOMAIN_MODEL.md)
- [Canonical matching and merge/split](architecture/CANONICAL_MATCHING.md)
- [Adapter platform](architecture/ADAPTER_PLATFORM.md)
- [Live provider model](architecture/LIVE_PROVIDER_MODEL.md)

## Product features

- [Product shell](features/PRODUCT_SHELL.md)
- [Product maturity](features/PRODUCT_MATURITY.md)
- [Daily-use UX](features/DAILY_USE_UX.md)
- [Responsive UI verification](features/RESPONSIVE_UI.md)
- [Development and production builds](features/BUILD_PROFILES.md)
- [Manga reader](features/MANGA_READER.md)
- [Anime player](features/ANIME_PLAYER.md)
- [Local library and backup](features/LOCAL_LIBRARY_AND_BACKUP.md)

## Providers

- [Source analysis](providers/SOURCE_ANALYSIS.md)
- [Live media reconnaissance](providers/LIVE_MEDIA_RECON.md)
- [Live provider media](providers/LIVE_PROVIDER_MEDIA.md)
- [Compatibility matrix](providers/LIVE_COMPATIBILITY_MATRIX.md)
- [Reliability](providers/LIVE_RELIABILITY.md)

## TV and release

- [Android TV / Google TV / Fire OS experience](tv/TV_EXPERIENCE.md)
- [Installing and updating production builds](release/INSTALLING.md)
- [1.0 RC.1 preparation and validation handoff](release/v1.0.0-rc.1.md)
- [Release hardening](release/RELEASE_HARDENING.md)
- [Maintainer release process](release/RELEASING.md)
- [v0.2.0-beta.1 release notes](release/v0.2.0-beta.1.md)
- [v0.2.0-beta.2 release notes](release/v0.2.0-beta.2.md)
- [v0.2.0-beta.3 release notes](release/v0.2.0-beta.3.md)
- [v0.2.0-beta.4 release notes](release/v0.2.0-beta.4.md)
- [Production Android signing](release/PRODUCTION_SIGNING.md)
- [beta.1 to beta.2 migration](release/BETA1_TO_BETA2_MIGRATION.md)
- [Fire TV validation status and checklist](release/FIRE_TV_VALIDATION.md)
